...
def create_nginx_container(root_secret: string):
    ...
    def get_latest_wordpress_build():
        subprocess.run(["curl", "-O", "https://wordpress.org/latest.tar.gz", "--output-dir", "./vols/websites/template"])
        subprocess.run(["tar", "-xf", "./vols/websites/template/latest.tar.gz", "-C", "./vols/websites/template"])
        subprocess.run(["rm", "./vols/websites/template/latest.tar.gz", "./vols/webistes/template/wordpress/xmlrpc.php"])
        subprocess.run(["chown", "-R", "www-data:www-data", "./vols/websites/template/wordpress"])
